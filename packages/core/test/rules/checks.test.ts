 ...
 * with the owner's profile. Severity is whatever the context says a
...
describe('checkCeiling', () => {
  ...
  it('takes the context severity for overlays', () => {
    const { c, ctx: x } = cond('VRB03KT P6SM OVC010', { violation: 'marginal', basis: 'TEMPO 15:00Z–17:00Z' });
    expect(checkCeiling(x, c)[0]!.severity).toBe('marginal');
  });
  it('vertical visibility is a ceiling; CAVOK is unlimited; no sky at all is an advisory', () => {
    const vv = cond('00000KT 1/4SM FG VV002');
    expect(checkCeiling(vv.ctx, vv.c)[0]!.severity).toBe('no-go');
    const cav = cond('24010KT CAVOK');
    expect(checkCeiling(cav.ctx, cav.c)[0]!.severity).toBe('ok');
    const none = cond('24010KT P6SM');
    expect(checkCeiling(none.ctx, none.c)[0]!.severity).toBe('advisory');
  });
});
...
describe('checkVisibility', () => {
  it.each([
    ...
    expect(checkVisibility(x, c)[0]!.severity).toBe(sev);
  });
  it('is an advisory when no visibility is given', () => {
    const { c, ctx: x } = cond('24010KT SKC');
    expect(checkVisibility(x, c)[0]!.severity).toBe('advisory');
  });
});
...
describe('checkCrosswind', () => {
  ...
  it('fails on the gust when the profile includes gusts, and cites the demonstrated figure too', () => {
    ...
    expect(fs.map((f) => [f.rule, f.severity])).toEqual([
      ['crosswind.personal', 'ok'],
      ['crosswind.demonstrated', 'ok'],
    ]);
  ...
  it('ignores the gust when the profile says so', () => {
    ...
    expect(checkCrosswind(x, c)[0]!.severity).toBe('ok');
  });
  ...
  it('is an advisory without an airport, a wind, or a direction', () => {
    ...
    expect(checkCrosswind(x, c)[0]!.severity).toBe('advisory');
    const { c: c2, ctx: x2 } = cond('/////KT P6SM SKC');
    expect(checkCrosswind(x2, c2)[0]!.severity).toBe('advisory');
...
describe('checkRegulatory', () => {
  it('evaluates CARs control-zone minima: visibility, 1,000 ft ceiling, and cloud clearance at cruise', () => {
    ...
    expect(fs.map((f) => [f.rule, f.severity])).toEqual([
      ['vfr.visibility', 'no-go'],
      ['vfr.ceiling', 'no-go'],
      ['vfr.cloudClearance', 'no-go'],
      ['vfr.cloudClearance', 'no-go'],
    ]);
  ...
  it('says so when airspace class is unknown or the country has no table', () => {
    ...
    expect(checkRegulatory(x, c)[0]).toMatchObject({ rule: 'vfr.minima', severity: 'advisory' });
  ...
  it('picks night rows for uncontrolled airspace at night', () => {
    ...
    expect(checkRegulatory(day.ctx, day.c)[0]!.severity).toBe('ok');
  });
});
...
describe('checkNight', () => {
  it('is silent by day, advisory at night, a violation when the profile forbids night', () => {
    ...
    expect(checkNight(ctx({ night: true }))[0]!.severity).toBe('advisory');
    const p = parsePilotProfile({ ceilingAglFt: 2500, visibilitySm: 5, crosswindKt: 15, nightAllowed: false });
    expect(checkNight(ctx({ night: true, profile: p }))[0]!.severity).toBe('no-go');
  });
});
